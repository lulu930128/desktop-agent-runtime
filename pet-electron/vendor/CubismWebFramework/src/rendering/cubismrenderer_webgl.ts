...
const DISABLE_CULLING_FOR_MIGRATION = true;
...
export class CubismRenderer_WebGL extends CubismRenderer {
  ...
  public drawObjectLoop(lastFbo: WebGLFramebuffer): void {
    ...
    // Sort by render-order value. Some Core versions expose order values that
    // are not safe to use as dense array indexes, which can invert/skip layers.
    const sortedObjects: Array<{
      order: number;
      index: number;
    ...
    for (let i = 0; i < drawableCount; ++i) {
      ...
      sortedObjects.push({
        ...
        index: i,
        type: DrawableObjectType.DrawableObjectType_Drawable
      });
    }

    for (let i = 0; i < offscreenCount; ++i) {
      const order = Number(renderOrder[drawableCount + i]);
      sortedObjects.push({
        order: Number.isFinite(order) ? order : drawableCount + i,
        index: i,
        type: DrawableObjectType.DrawableObjectType_Offscreen
      });
    }
    ...
    this._sortedObjectsIndexList.length = 0;
    this._sortedObjectsTypeList.length = 0;

    // 描画
    for (const object of sortedObjects) {
      this._sortedObjectsIndexList.push(object.index);
      this._sortedObjectsTypeList.push(object.type);
      this.renderObject(object.index, object.type);
    }
  ...
  public drawMeshWebGL(model: Readonly<CubismModel>, index: number): void {
    // 裏面描画の有効・無効
    if (DISABLE_CULLING_FOR_MIGRATION) {
      this.gl.disable(this.gl.CULL_FACE);
    } else if (this.isCulling()) {
