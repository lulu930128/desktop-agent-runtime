...
const ENABLE_ADVANCED_BLEND_SHADERS = false;
const FORCE_LEGACY_BLEND_RENDER_PATH = true;
...
export class CubismShader_WebGL {
  private resolveBlendEnumValue(rawValue: unknown): number | null {
  ...
  public constructor() {
    ...
    for (let i = 0; i < colorBlendKeys.length; i++) {
      ...
      if (colorBlendKey.includes(ColorBlendPrefix)) {
        ...
        const colorBlendNumber = this.resolveBlendEnumValue(
          colorBlendRawValues[i]
        );

        if (colorBlendNumber == null) {
          continue;
        }
    ...
    for (let i = 0; i < alphaBlendKeys.length; i++) {
      ...
      if (alphaBlendKey.includes(AlphaBlendPrefix)) {
        ...
        const alphaBlendNumber = this.resolveBlendEnumValue(
          alphaBlendRawValues[i]
        );

        if (alphaBlendNumber == null) {
          continue;
        }
    ...
    console.info('[pet-renderer] Shader blend support summary', {
      colorBlendCount: this._colorBlendValues.length,
      alphaBlendCount: this._alphaBlendValues.length,
      shaderCount: this._shaderCount
    });
    // シェーダーの数 =
          ...
          default:
            {
              ...
              const blendShaderKey =
                this._colorBlendMap.get(colorBlendMode) +
                this._alphaBlendMap.get(alphaBlendMode);
              const baseShaderSetIndex =
                this._blendShaderSetMap.get(blendShaderKey);
              const fallbackShaderIndex =
                ShaderNames.ShaderNames_NormalPremultipliedAlpha + offset;

              if (
                baseShaderSetIndex == null ||
                this._shaderSets[baseShaderSetIndex + offset] == null
              ) {
                console.warn(
                  '[pet-renderer] Missing advanced drawable blend shader, falling back to normal blend',
                  { blendShaderKey, colorBlendMode, alphaBlendMode }
                );
                shaderSet = this._shaderSets[fallbackShaderIndex];
                srcColor = this.gl.ONE;
                dstColor = this.gl.ONE_MINUS_SRC_ALPHA;
                srcAlpha = this.gl.ONE;
                dstAlpha = this.gl.ONE_MINUS_SRC_ALPHA;
  ...
  public setupShaderProgramForOffscreen(
    ...
    if (
      FORCE_LEGACY_BLEND_RENDER_PATH ||
      colorBlendMode == CubismColorBlend.ColorBlend_None ||
      alphaBlendMode == CubismAlphaBlend.AlphaBlend_None ||
      (colorBlendMode == CubismColorBlend.ColorBlend_Normal &&
        alphaBlendMode == CubismAlphaBlend.AlphaBlend_Over)
        ...
        default:
          {
            ...
            if (
              ...
              console.warn(
                '[pet-renderer] Missing advanced offscreen blend shader, falling back to normal blend',
                { blendShaderKey, colorBlendMode, alphaBlendMode }
              );
              shaderSet = this._shaderSets[fallbackShaderIndex];
              srcColor = this.gl.ONE;
    ...
    if (shaderSet == null) {
      console.warn(
        '[pet-renderer] Offscreen shaderSet was unresolved, forcing normal premultiplied shader',
        { offset, colorBlendMode, alphaBlendMode }
      );
      shaderSet =
        this._shaderSets[ShaderNames.ShaderNames_NormalPremultipliedAlpha + offset];
  ...
  public generateShaders(): void {
    ...
    this.loadShaders()
      .then(() => {
        ...
        if (ENABLE_ADVANCED_BLEND_SHADERS) {
          this.registerBlendShader(); // ブレンドモードシェーダーの登録
        } else {
          console.info('[pet-renderer] Skipping advanced blend shader registration');
        }
        this._isShaderLoading = false;
        this._isShaderLoaded = true;
        console.info('[pet-renderer] Shader programs loaded', {
          shaderCount: this._shaderCount
        });
      })
      .catch(error => {
        this._isShaderLoading = false;
        console.error('[pet-renderer] Failed to load shaders:', error);
      });
  }
